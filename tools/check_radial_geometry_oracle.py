#!/usr/bin/env python3
"""Independent pixel-exact oracle for ODPAR 0.13 radial field lines."""
...
def expected():
 ...
 radial=[(Q*((i*17+5)%49))//48 for i in range(48)]
 for i,rv in enumerate(radial):
  ph=(RING_PHASE+((i<<32)//48))&MASK32;cs=cosq(ph);sn=sinq(ph);span=4<<16;ln=(span*rv+Q//2)//Q;r0=radius+gap;r1=r0+ln
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='odm-radial-oracle-') as td0:
 ...
 print('RADIAL GEOMETRY ORACLE: PASS');print(f'  segments: 48, pixels={W*H}, channels={W*H*4}');print('  independently reconstructed: CORDIC + endpoints + Q24.8 projection + coverage + premul blend');print('  stream sha256='+hashlib.sha256(exp).hexdigest());return 0
