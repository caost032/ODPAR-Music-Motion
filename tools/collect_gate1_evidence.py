#!/usr/bin/env python3
"""Execute Gate 1 verification and emit an honest deterministic receipt."""
...
def embedded_json(data: bytes, schema: str) -> dict[str, object] | None:
    for raw_line in reversed(data.decode("utf-8", errors="replace").splitlines()):
        line = raw_line.strip()
...
def run_step(root: Path, name: str, command: list[str]) -> dict[str, object]:
    ...
    benchmark = embedded_json(normalized_stdout, "odm_core_benchmark_v1")
...
def main() -> int:
    ...
    local_steps = [
        ...
        (
            "release_performance_floors",
            [
                "make",
                "BUILD_DIR=build/bench-gcc",
                "CC=gcc",
                "benchmark-core",
            ],
        ),
    ...
    observations = [
        run_step(root, name, command) for name, command in local_steps
    ]
    clang_path = shutil.which("clang")
    if clang_path:
        observations.append(
            run_step(root, "strict_clang_matrix", ["make", "test-clang"])
        )
        observations.append(
            run_step(root, "asan_ubsan_clang", ["make", "asan-clang"])
        )
    else:
        for name, target in (
            ...
            blocked["reason"] = (
                "clang executable not found; Make target failed closed"
            )
            observations.append(blocked)

    local_observations = observations[: len(local_steps)]
    local_pass = all(item.get("status") == "pass" for item in local_observations)
    clang_observations = observations[len(local_steps) :]
    ...
    receipt: dict[str, object] = {
        ...
        + [
            ...
            "Performance floors describe this evidence host and are not a cross-platform speed certification.",
        ],
    }
    ...
    print(
        f"Gate 1 evidence: local={receipt['local_status']} "
        f"full={receipt['full_gate1_status']} output={output}"
    )
