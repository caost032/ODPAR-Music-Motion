...
void odm_test_status(odm_test_context *context) {
    ...
    ODM_TEST_CHECK(context, ODM_VERSION_MINOR == 24u);
    ODM_TEST_CHECK(context, ODM_VERSION_PATCH == 0u);
    ODM_TEST_CHECK(context, strcmp(ODM_VERSION_LABEL, "sample-accurate-causal-presentation-v1-wip-local") == 0);
    ODM_TEST_CHECK(context, strcmp(odm_version_string(), ODM_VERSION_STRING) == 0);
    ODM_TEST_CHECK(context, strcmp(ODM_VERSION_STRING, "0.24.0-sample-accurate-causal-presentation-v1-wip-local") == 0);
}
